...
let brightToFcTable = [], minLPFc = 50, maxLPFc = 20000, minHPFc = 0, maxHPFc = 15000, allPassZone = 0.05, brThresholdLP = MAX_BRIGHTNESS * (0.5 - allPassZone), brThresholdHP = MAX_BRIGHTNESS * (0.5 + allPassZone), allpass = false, filterFc, filterType, brightness = 0;
...
// const socket = new StayAliveSocket("ws://localhost:5000");
socket.onOpen = (event) => {
    console.log("open");
    const message3 = {
        type: "GetOrd",
    ...
    if (message.type === "Ord") {
        ...
        if (localStorage.getItem("trialConfiguration") !== null) {
            ...
            config = {
                ...
                trialConfig: message.data === "Sound"
...
const practise1NextButton = document.querySelectorAll("[next-practise-1-button]");
const practise2NextButton = document.querySelectorAll("[next-practise-2-button]");
...
const participantinfo1NextButton = document.querySelectorAll("[next-participant-info-1-button]");
...
participantinfo1NextButton.forEach((button) => {
    button.addEventListener("click", () => {
        ...
        const gender = document.getElementById("gender-select")
            .value;
        const countryChildhod = (document.getElementById("childcountry")).value;
        const countryCurrent = (document.getElementById("currentcountry")).value;
...
participantinfo2NextButton.forEach((button) => {
    button.addEventListener("click", () => {
        const musicianship = (document.getElementById("musicianship")).value;
        ...
        const fourier = document.getElementById("Fourier")
            .value;
        const threed_experience = (document.getElementById("3d-experience")).value;
...
function createGeometry() {
    ...
    // params.height,
    // params.depth,
    2, 2, 2, params.Segments, params.Segments, params.Segments);
    ...
    geometry.morphAttributes.position[2] = new THREE.Float32BufferAttribute(sinpositions, 3);
    return geometry;
}
function initGUI() {
...
function initBrightnessTable() {
    ...
    for (br = 0; br <= MAX_BRIGHTNESS; br++) {
        if (br <= brThresholdLP) {
            ...
            brightToFcTable[br] =
                minHPFc +
                    Math.pow(maxHPFc - minHPFc, (br - brThresholdHP) / (MAX_BRIGHTNESS - brThresholdLP));
        }
        else {
...
function createEnv() {
    ...
    normGain.gain.linearRampToValueAtTime(0.8 / 4, this.onsetTime + attackTime + decayTime);
    normGain.gain.setValueAtTime(0.8 / 4, this.onsetTime + attackTime + decayTime + sustainTime);
    normGain.gain.linearRampToValueAtTime(0, this.onsetTime + attackTime + decayTime + sustainTime + releaseTime);
}
...
function main(config) {
    ...
    sound.setNodeSource(normGain); //typescript getting confused by threejs audio types
